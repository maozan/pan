...
# /home/hwt/.cache/torch/hub/checkpoints/
...
torch.backends.cudnn.benchmark = True # accelerate the neural network
...
def train(epoch):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        PAN_edge    = Edge(PAN_image.unsqueeze(dim=1).detach().cpu().numpy())
        PAN_edge    = torch.from_numpy(PAN_edge).float().cuda()
        # print(PAN_image.dtype, PAN_edge.dtype)
        out         = model(MS_image, PAN_image, PAN_edge)
        ...
        if config[config["train_dataset"]]["Transfer_Periferal_Loss"]:
            # 1. 这里存在一个问题，弱多卡训练，实际上是把数据拆分成两份后到不同的gpu计算，然后都到gpu0上合并，但是
            # 2. 在多卡训练的forward中计算loss时候，loss好像没有被合并(见第3点解释)，因此多卡情况下out["tp_loss"]返回两个值
            # 3. 或许多卡计算完毕merge的时候，是按0维度concat的，因为 [4, 102, 40, 40] * 2 ---> [8, 102, 40, 40]
            # 4. 解决办法好像就是，不多卡训练，或者用loss.mean()，即out["loss_tp"].mean()，但是这样不知道会不会对训练的精度有影响
        ...
        # Spatial loss
        if args.config == 'configs/config_HSIT.json':
