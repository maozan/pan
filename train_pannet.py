...
from utils.high_pass import highPass
...
def train(epoch):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        MS_hp, PAN_hp   = highPass(PAN_image.unsqueeze(dim=1).detach().cpu().numpy(), MS_image.detach().cpu().numpy()) 
        out             = model(MS_image, PAN_image, MS_hp, PAN_hp)
